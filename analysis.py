...
from premium_theme import apply_plotly_theme, PRIORITY_COLOR_PREMIUM
from config import MRU_COLORS, CHARGED_COLOR, UNCHARGED_GREY
...
def run_analysis(df_conn, df_master, show_deep_tabs=True, widget_prefix="analysis", show_map_section=True,
                 d0=None, d1=None, map_mode="heatmap_connections", heatmap_tile="road",
                 anim_mrus=None, show_charged_overlay=False, show_uncov_overlay=False):
    df_c = df_conn.copy()
    df_m = df_master.copy()

    df_c["MRU"] = df_c["MRU"].fillna("Unassigned").astype(str).str.strip()
    df_m["MRU"] = df_m["MRU"].fillna("Unassigned").astype(str).str.strip()
    df_c["Main_Area"] = df_c["Main_Area"].fillna("").astype(str).str.strip()
    df_m["Main_Area"] = df_m["Main_Area"].fillna("").astype(str).str.strip()
    df_c["Subarea"] = df_c["Subarea"].fillna("Unassigned").astype(str).str.strip()
    df_m["Subarea"] = df_m["Subarea"].fillna("Unassigned").astype(str).str.strip()

    df_c = df_c[df_c["MRU"] != "Unassigned"].copy()
    df_m = df_m[df_m["MRU"] != "Unassigned"].copy()
    df_m["Conversion Date"] = pd.to_datetime(df_m["Conversion Date"], errors="coerce")

    ALL_MRUS = sorted(df_c["MRU"].dropna().astype(str).unique())
    if anim_mrus is None:
        anim_mrus = ALL_MRUS

    # tile config — always computed so bubbles + heatmaps all use the same selector
    HEATMAP_TILE_URLS = {
        "satellite": "https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}",
        "road":      "https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}",
        "terrain":   "https://mt1.google.com/vt/lyrs=p&x={x}&y={y}&z={z}",
    }
    if heatmap_tile == "dark":
        map_mapbox_style = "carto-darkmatter"
        map_tile_layer   = []
    else:
        map_mapbox_style = "white-bg"
        map_tile_layer   = [
            {"sourcetype": "raster", "sourceattribution": "Google",
             "source": [HEATMAP_TILE_URLS.get(heatmap_tile, HEATMAP_TILE_URLS["road"])],
             "below": "traces"}
        ]
    # keep old names as aliases so existing heatmap code paths still work
    heatmap_mapbox_style = map_mapbox_style
    heatmap_tile_layer   = map_tile_layer
    ...
    # date range fallback if not passed
    ...
    if d0 is None or d1 is None:
        if pd.notna(min_date) and pd.notna(max_date):
            d0, d1 = min_date.date(), max_date.date()
        else:
            d0, d1 = None, None
    ...
    # BUILD MAP
    # ════════════════════════════════════════════════════════
    _map_legend_html = ""  # only set for heatmap modes; rendered inside show_map_section
    if map_mode == "dots_by_mru":
        ...
        if df_anim.empty:
            ...
            if getattr(afig.layout, "updatemenus", None) and len(afig.layout.updatemenus) > 0:
                buttons = getattr(afig.layout.updatemenus[0], "buttons", None)
                if buttons and len(buttons) > 0 and len(buttons[0].args) > 1:
                    frame_args = buttons[0].args[1]
                    if "frame" in frame_args:
                        frame_args["frame"]["duration"] = 600
                    if "transition" in frame_args:
                        frame_args["transition"]["duration"] = 300
            afig.update_traces(marker=dict(size=5, opacity=0.85))
    else:
        afig = go.Figure()
    ...
    if show_charged_overlay and map_mode not in ("heatmap_connections","heatmap_charged","time_animation"):
        ...
        tr = ov1.data[0]
        tr.marker.size=7.2; tr.marker.opacity=0.98
    ...
    if show_map_section:
        ...
        st.plotly_chart(afig, use_container_width=True, config={"scrollZoom":True}, key=f"{widget_prefix}_map_chart")
        if _map_legend_html:
    ...
    with t1:
        ...
        with ctrl_l:
            gran = st.radio("Granularity", ["Daily","Weekly","Monthly"], horizontal=True, key=f"{widget_prefix}_granularity")
        ...
        with ctrl_r:
            ...
            sel_s = st.multiselect("Subareas", sorted(df_td2["Subarea"].unique()), default=def_s, key=f"{widget_prefix}_subareas")
        ...
        with ca:
            ...
            st.plotly_chart(fig_l, use_container_width=True, key=f"{widget_prefix}_trend_cumulative")
        with cb:
            ...
            st.plotly_chart(fig_wm, use_container_width=True, key=f"{widget_prefix}_trend_velocity")
        ...
        with ca2:
            ...
            st.plotly_chart(fig_dow, use_container_width=True, key=f"{widget_prefix}_trend_weekday")
        with cb2:
            ...
            st.plotly_chart(fig_mom, use_container_width=True, key=f"{widget_prefix}_trend_mom")
    ...
    with t2:
        ...
        st.plotly_chart(fig_bass, use_container_width=True, key=f"{widget_prefix}_bass_chart")
    ...
    with t3:
        ...
        if not fc_plot.empty:
            ...
            st.plotly_chart(fig_eta, use_container_width=True, key=f"{widget_prefix}_time_to_50_chart")
        st.dataframe(fc.style.format({"Adoption %":"{:.1f}%"}), use_container_width=True)

    # ── SUBAREA SCORECARD — below the tabs, Tab 3 (Analysis) only ──
    st.markdown("<div style='height:1px;background:rgba(255,255,255,0.07);margin:1.75rem 0 0.5rem'></div>", unsafe_allow_html=True)
    st.markdown("""
    <div style="display:flex;align-items:center;gap:8px;margin:1rem 0 0.75rem">
        <div style="width:6px;height:6px;border-radius:50%;background:#0071e3"></div>
    ...
    for _, row in area_d.sort_values("adoption_d", ascending=False).iterrows():
        ...
        adp = row["adoption_d"]
        adp_color = "#30d158" if adp >= 50 else "#ff9f0a" if adp >= 20 else "#ff453a"
        cols[4].markdown(f"<span style='color:{adp_color};font-weight:700;font-size:0.875rem'>{adp:.1f}%</span>",
                         unsafe_allow_html=True)
